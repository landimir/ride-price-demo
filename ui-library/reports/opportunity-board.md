# Ride Price Mobile UI — Opportunity Board

Updated 2026-08-23 · 1 of 20 product areas reviewed · 6 recommendations so far. This board is generated from the screen improvement matrix and grows as each area is reviewed — until every area is in, "top" means top of what has been reviewed.

## Top opportunities (by priority, then severity, then how widely they repeat)

| # | Id | Area | Opportunity | Priority | Severity | Size |
|---|---|---|---|---|---|---|
| 1 | RP-IMP-001 | Home — Deals Queue & Navigation | The only forward action sits in the hardest thumb zone while 60% of the screen is empty | High | Major | medium |
| 2 | RP-IMP-002 | Home — Deals Queue & Navigation | Finished deals hide behind a 20px grey caption | High | Minor | small |
| 3 | RP-IMP-003 | Home — Deals Queue & Navigation | The card does not say what comes next, and the search-field camera promises the wrong scan | Medium | Minor | small |
| 4 | RP-IMP-004 | Home — Deals Queue & Navigation | The drawer spends the brand gradient on a selected state | Low | Minor | small |
| 5 | RP-IMP-006 | Home — Deals Queue & Navigation | No login by design — keep the 'who am I' cue consistent | Low | Observation | small |
| 6 | RP-IMP-005 | Home — Deals Queue & Navigation | A filter with no matches contradicts the title and offers no way back | Low | Observation | small |

## Which areas need the most attention

| Area | Recommendations | High priority |
|---|---|---|
| Home — Deals Queue & Navigation | 6 | 2 |

## Issues that repeat across screens

- **RP-IMP-001** — The only forward action sits in the hardest thumb zone while 60% of the screen is empty — on 7 screens (Home — Deals Queue & Navigation; Customer Onboarding — Find a Customer; Training Licenses & Registrations)
- **RP-IMP-003** — The card does not say what comes next, and the search-field camera promises the wrong scan — on 4 screens (Home — Deals Queue & Navigation)
- **RP-IMP-004** — The drawer spends the brand gradient on a selected state — on 2 screens (Home — Deals Queue & Navigation)
- **RP-IMP-006** — No login by design — keep the 'who am I' cue consistent — on 2 screens (Home — Deals Queue & Navigation)

## Biggest overall gains

- One fixed home for the forward action on phones (a bottom bar) — it fixes Home's thumb reach now and is the same change the desking, credit-application and finance-menu Majors will need later.
- 'Done' as a first-class filter and 'Next ·' on the card — two small changes that make the home screen answer a manager's and an advisor's first questions without a tap.

## Where to start

1. RP-IMP-002 (Funded pill / 48px archive row) — small, closes RP-UI-017, no layout decision needed.
2. RP-IMP-003 and RP-IMP-005 — copy-level changes on the landing card and the empty state.
3. RP-IMP-001 — the bottom action bar, once the owner has decided against his one-row title + action sample on phones.
